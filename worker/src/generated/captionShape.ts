...
/** Why a caption produced no shape. A shape and a reason are mutually
 *  exclusive, and 0196 enforces that with a CHECK rather than trusting code. */
export type NoShapeReason =
  | 'no_title' | 'empty_after_strip' | 'too_short' | 'not_english' | 'no_pattern_match'

export interface CaptionVerdict {
  shape: CaptionShape | null
  basis: 'inferred' | null
 ...
 *
 * ⚠️⚠️ MOVED HERE FROM `scripts/backfill-caption-shapes.ts` ON 2026-09-20, AND
 * THE MOVE IS THE POINT. It lived in a manual script, which made the script the
 * only thing that could classify anything — so `caption_shape` froze on
 * 2026-09-10 at 596 rows while `gallery_items` grew to 6,423 and the shape block
 * in `generate-blueprint` kept reading the stale snapshot on every generation.
 * The rule belongs beside the patterns it gates, where the backfill script, the
 * worker sweep and any future reader all reach ONE author.
 *
 ...
 * this split a 10-character English caption is recorded as `not_english`, which
 * is a false fact in the column whose whole purpose is to be trustworthy.
 */
export function classifyOne(title: string | null | undefined): CaptionVerdict {
  ...
  // ⚖️ `classifyCaption`, NOT `assessedCaptionShape`. The assessed wrapper needs
  // a timestamp this function has no use for, and the script that used to own
  // this code called it with the argument missing — harmless only because it
  // then read `.value` and nothing else. The primitive says what is meant.
  const read = classifyCaption(t)
  if (read === null) return { shape: null, basis: null, reason: 'no_pattern_match' }
  return { shape: read.shape, basis: 'inferred', reason: null }
}
