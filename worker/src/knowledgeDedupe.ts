...
//
// ⚠️ A COPY OF `packages/shared/src/knowledgeDedupe.ts`. The worker has no
// runtime dependency on the shared package; `knowledgeDedupeParity.test.ts`
 ...
 *
 *  ⚠️ MUST EQUAL `SUBSTANCE_KINDS` in `packages/shared/src/knowledgeSelection.ts`.
 *  The worker has no runtime dependency on the shared package, so this is a copy
 *  and `knowledgeDedupeParity.test.ts` is what keeps it one. */
