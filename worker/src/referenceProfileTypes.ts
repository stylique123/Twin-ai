// ⚠️ DERIVED FROM `packages/shared/src/referenceContentProfile.ts`.
// See referenceExtraction.ts for why. Parity is enforced by test.
...
import { unchecked } from './assessedTypes.js'

/** ⚠️ THE CTA VOCABULARY, COPIED FROM `packages/shared/src/cta.ts` RATHER THAN
 *  IMPORTED, for the same reason as everything else in this file. The parity
 *  test compares it against that module, not against a second opinion. */
