...
import {
  ...
  SlidersHorizontal, Type, Palette, Music, Clapperboard, Zap, X,
} from 'lucide-react'
import { getGeneration, autoEditTake, remakeEdit, getJob, updateGenerationChoice, fetchEdl, reEditWithEdl } from '../lib/api'
...
export default function Record() {
  ...
  const openRefine = async () => {
    if (!edlPath) { setRefineEdl(null); setRefineOpen(true); return }
    setRefineLoading(true)
    try {
      const e = await fetchEdl(edlPath)
      setRefineEdl(e)
    } catch { setRefineEdl(null) } finally { setRefineLoading(false); setRefineOpen(true) }
  }

  // ---- Refine: apply the edited EDL and re-render (free) ----
  const applyRefine = async () => {
    if (!refineEdl || !takePathRef.current || !id) return
    setRefineOpen(false)
    setEditErr(null)
    setEditPhase('working')
    setEditStatus('Applying your changes…'); setEditPct(3)
    try {
      const jobId = await reEditWithEdl(id, takePathRef.current, refineEdl)
      if (editUrl) URL.revokeObjectURL(editUrl)
      setEditUrl(null)
      await pollEdit(jobId)
    } catch (e) {
      setEditErr(e instanceof Error ? e.message : 'Refine failed.')
      setEditPhase('error')
    }
  }
  // Helpers to mutate the in-panel EDL copy.
  const setCapStyle = (style: string) => setRefineEdl((e) => e ? { ...e, captions: { ...e.captions, style } } : e)
  const setCapColor = (v: number) => setRefineEdl((e) => e ? { ...e, variation: v, captions: { ...e.captions, variation: v } } : e)
  const setMusic = (on: boolean) => setRefineEdl((e) => e ? { ...e, music: on } : e)
  const setEnergy = (en: 'high' | 'calm') => setRefineEdl((e) => e ? { ...e, energy: en } : e)
  const setBrollKeep = (keep: boolean) => setRefineEdl((e) => e ? { ...e, broll: keep ? (e.broll ?? { query: 'b-roll', start: 2, end: 4.2 }) : null } : e)
  const setBrollQuery = (q: string) => setRefineEdl((e) => e && e.broll ? { ...e, broll: { ...e.broll, query: q } } : e)
  ...
  return (
    <main className="relative mx-auto max-w-6xl px-5 py-8 lg:py-10">
      ...
      {/* ---- Refine panel: the easy 20% manual layer (free re-render) ---- */}
      <AnimatePresence>
        {refineOpen && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-end justify-center bg-ink/70 p-0 backdrop-blur-sm sm:items-center sm:p-5"
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setRefineOpen(false)} className="grid h-9 w-9 place-items-center rounded-xl border border-white/10 bg-white/5 text-stone hover:text-cream"><X className="h-4 w-4" /></button>
              </div>
              ...
              {!refineEdl ? (
                ...
                <div className="mt-5 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CAPTION_STYLE_OPTIONS.map((s) => (
                        <button key={s.id} onClick={() => setCapStyle(s.id)}
                          className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-colors ${refineEdl.captions.style === s.id ? 'border-coral/60 bg-coral/15 text-cream' : 'border-white/10 bg-white/5 text-stone hover:text-cream'}`}>
                          {s.label}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2.5">
                      {CAPTION_COLOR_OPTIONS.map((c) => (
                        <button key={c.id} onClick={() => setCapColor(c.id)} title={c.label}
                          className={`h-9 w-9 rounded-full ring-2 ring-offset-2 ring-offset-ink transition-all ${refineEdl.variation === c.id ? 'ring-cream scale-110' : 'ring-transparent hover:ring-white/30'}`}
                    ...
                    <div className="flex gap-2">
                      {(['calm', 'high'] as const).map((en) => (
                        <button key={en} onClick={() => setEnergy(en)}
                          className={`flex-1 rounded-card border px-3 py-2 text-sm font-medium capitalize transition-colors ${refineEdl.energy === en ? 'border-coral/60 bg-coral/15 text-cream' : 'border-white/10 bg-white/5 text-stone hover:text-cream'}`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setMusic(!refineEdl.music)}
                      className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-colors ${refineEdl.music ? 'border-teal/50 bg-teal/15 text-cream' : 'border-white/10 bg-white/5 text-stone'}`}>
                      {refineEdl.music ? 'On' : 'Off'}
                    </button>
                  </div>
                  {/* B-roll */}
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => setBrollKeep(!refineEdl.broll)}
                        className={`rounded-full border px-3.5 py-1.5 text-xs font-medium transition-colors ${refineEdl.broll ? 'border-teal/50 bg-teal/15 text-cream' : 'border-white/10 bg-white/5 text-stone'}`}>
                        {refineEdl.broll ? 'On' : 'Off'}
                      </button>
                    </div>
                    {refineEdl.broll && (
                      <input className="field mt-2 text-sm" value={refineEdl.broll.query} onChange={(e) => setBrollQuery(e.target.value)} placeholder="what the b-roll should show" />
                    )}
                  </div>

                  <button onClick={applyRefine} className="btn-gradient w-full">
                    <Sparkles className="h-4 w-4" /> Apply &amp; re-render (free)
                  </button>
                </div>
              )}
            </motion.div>
