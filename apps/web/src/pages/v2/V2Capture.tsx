...
function CaptureGate({ genId, mode, onBack }: { genId: string; mode: 'upload' | 'record'; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // ⚖️ FETCHED ONCE AND SHARED, LAZILY. Both `selectedHook` and
        // `synthScript` need the generation row; memoising keeps entering record
        // mode at one read instead of two, and calling it only from inside
        // `prepareCaptureMode` keeps UPLOAD at zero reads, which that module
        ...
        const r = await prepareCaptureMode(mode, {
          ...
          // camera reads a line she did not choose — the defect #927 is named
