// Source-invariant parity (Constitution §10D): the source-asset edge (Deno)
// cannot import shared at deploy time, so it INLINES the edge-core functions.
// Shared is the single source of truth; this test proves the edge's inlined
// bodies are byte-identical (whitespace-normalized) to the shared canonical
// definitions, so they can never silently drift.
...
// Extract a top-level `export function NAME(...) { ... }` up to its column-0 `}`.
...
describe('edge ↔ shared source-invariant parity (no drift)', () => {
  it('the edge inlines each edge-core function byte-identically to shared', () => {
    // The edge inlines under EDGE-CORE markers; confirm they exist.
