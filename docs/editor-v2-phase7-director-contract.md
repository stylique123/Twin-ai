...
- `selections[]`: a bare array of candidate **indices** (`int[]`) — no per-selection object, no per-selection `reason`. The model has no field in which to argue for a cut; the index is the entire wire. Re-resolve each index against `envelope.candidates`: out-of-range/fabricated → `director_decision_bad_ref`. Read kind/selectionEnabled/span **from the pinned envelope tuple**, never from the model. Reject `selectionEnabled !== 1` and `kind === 'filler'` → `director_decision_filler`. Reject duplicate indices.
...
- `summary` is the ONLY free text the model may return, capped at `MAX_DECISION_SUMMARY_CHARS` **JS string length** (not bytes) and stored as inert data — never interpreted (prompt-injection containment: the model cannot widen its own authority because only indices/enums are consumed). Because the cap is on characters, the worst-case *byte* size of a response is set by multi-byte/escape-heavy text, not by ascii; `MAX_DECISION_OUTPUT_BYTES` is derived from that true worst case and pinned by a test.
