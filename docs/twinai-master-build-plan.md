...
| 2026-08-05 | **Phase 12 item 13 (screen recording) — the SERVER half is built and the CLIENT half is BLOCKED, on something worth writing down.** 0107 adds `editor_create_clip_asset` / `editor_finalize_clip` (clients cannot write `media_assets` — 0076 revokes it), and a `clip-asset` edge function runs create → upload → finalize. A clip skips the source's whole capture contract, because that ceremony exists to prove the take is the take of the script the creator read and a screen recording is neither read from a script nor the thing the editor cuts. What it KEEPS is the idempotency backbone: a screen capture is the most-retried thing in the product (picker cancelled, wrong window, tab refreshed), so one attempt is one asset. Gate-K proves it and **caught a real bug in the RPC on its first run** — `status` in a WHERE clause resolving to the `RETURNS TABLE` output rather than the column, which is a runtime ambiguity error rather than a create-time one. **THE BLOCKER: the renderer cannot composite a second video source.** `EditPlan.identity.sourceAssetId` is singular and `ffmpegGraph.ts` builds every filter chain from input `0:v`/`0:a`. So "split screen or a cutaway?" is not a preference to choose between — NEITHER IS EXPRESSIBLE, and a recorded clip would be stored correctly and then dropped at render. **And nothing asks the question**: `can_record_screen` appears in no onboarding question and no screen, so with the correct `isExplicitlyTrue` gate (which `capabilities.ts` names this exact feature as the example for) the surface would be invisible to every user. The DNA's observed `formats` is a real signal for seeding the question but cannot answer it — §8a.2's observed-is-not-stated rule. *The order this needs:* ask the question first (camera-free, Phase 10 item 2's remainder), then decide the composition model and give the EditPlan a second source, and only then point a capture UI at slots the script actually declares. **The UI was drafted and then DELETED rather than shipped**: it derived its slots from `b_roll` shots because nothing emits `[SHOW: …]`, which is inference dressed as a decision. The server half ships storable-but-unconsumed, following 0103's precedent exactly — and 0107 is deliberately NOT applied to production, because a migration with no consumer is a change to a live database for nothing. |
