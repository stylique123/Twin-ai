...
>
> **Last substantive change: 2026-08-06. Since then, 561 pull requests have merged to
...
12. Container resolution with fallbacks; nothing unresolved reaches filming
    **BUILT** — `containerResolution.ts` + `UnfilledContainers.tsx`.
    **THE LEAK WAS REAL AND WAS BEING READ ALOUD.** Two independent placeholder
    repairers existed (`Result.tsx`, `recordingScriptAdapter.ts`) with
    near-identical regexes, and BOTH only matched a bracket token that was the
    ENTIRE line. So `"Today I want to talk about [product name]"` was a
    placeholder to neither, survived both, reached the teleprompter and was read
    into the camera. The two had also drifted — one trimmed and one did not, so
    a line with a leading space was a placeholder on the plan screen and real
    dialogue in the recorder. They are now one authority.
    **A DECLARED CLIP IS NOT AN UNRESOLVED SLOT.** `[SHOW: …]` is bracket-shaped
    and is the deliberate marker item 11 is built on. A resolver that stripped
    every bracket would delete it while reporting success — and the old
    whole-line rule would have replaced a lone `[SHOW: …]` with the hook text.
    Classified separately, matched case-insensitively (a marker that only counts
    in capitals silently becomes a gap), and left exactly as written.
    **ONLY THE HOOK IS FILLED**, because it is the only slot whose correct value
    this system holds. Everything else is SHOWN, never guessed: §2.3's absolute
    rule is that an unverified fact never reaches a script, and the alternative
    to a visible `[product name]` is an invented product name — the confidently
    wrong recommendation the plan calls the most expensive failure it can
    produce. There is deliberately no auto-fill button.
    **It reports rather than blocking the record button.** A hard gate would
    strand a creator whose script has one stray bracket, with the recorder
    refusing and nothing to do about it — and stranding someone at the camera is
    what Phase 11 exists to prevent. The gap is named ("*'product name' is a gap
    nothing filled*") on the plan screen beside the script editor that fixes it
    in one edit. A block they cannot clear teaches them to route around it.
    *Still to build:* the FALLBACKS — §2.3's three routes (brand-led,
    reference-led, research-led) are what would fill a container rather than
    report it, and none of them exists. This closes the "nothing unresolved
    reaches filming" half and declares the other half rather than implying it.
...
**PARTLY BUILT — the cover and the caption were already there; this is placement
plus the guarantee that was missing.**

**GENERATE-ONCE WAS A UI CONVENTION, NOT A GUARANTEE.** `generate-thumbnail`
had NO idempotence check: every invoke called the paid image model and wrote a
new `Date.now()`-suffixed object. The only thing preventing a second call was a
client-side `!gen.ai_thumb_path` conditional deciding whether to render the
button — defeated by a second tab, a stale cache, a direct invoke, or the moment
before the generation loads. Each miss cost a paid call AND silently replaced
the creator's cover. The function now short-circuits on a stored path: re-signs,
does not call the model, does not write. There is deliberately no `force`.
**And viewing does not invoke it at all.** An existing cover is signed
CLIENT-SIDE via `signEditUrls` — covers are stored at `${user.id}/ai-thumb/…`
precisely so the owner can sign them. That closes the deploy window: the edge
function auto-deploys on merge while the web app deploys from the same push, so
a new client can meet an old function, and in that window an "open the cover"
call would have regenerated it. Viewing a cover is a read, so it goes down a
read path; the function is invoked only to MAKE the first one.
**The cover moved behind a button.** It was a permanent 9:16 card on the plan
screen — a thing wanted exactly twice (look at it, download it on the way to
posting) occupying a screen read while filming. Now a button beside Download,
opening a dialog with the image and a download link.
**Caption + hashtags copy SEPARATELY**, because the platforms take two fields
and Instagram creators routinely put tags in the first comment — a combined copy
made them hand-delete tags every time. Each button reports its own copy, since a
shared flag would tell the creator their hashtags are on the clipboard when the
caption is. The row also shows length against the platform's own published
limit: a checkable fact about the caption, never advice about what length
performs (§7c's honesty line, applied to the caption box). A platform not in the
map shows nothing rather than a guessed limit, because a wrong limit would have
the creator trimming a caption that fits.
**SCHEDULING WIRED FROM THE PLAN SCREEN.** Everything needed already existed —
`schedulePost`, the calendar, the `publish_due` cron — and the plan screen was
simply not connected to it: `best time: 5:00 PM` rendered as DEAD TEXT beside a
"Mark as posted" button that only works after the fact. Each publish row now has
a Schedule button.
**The prefill is a suggestion and the screen says so.** `best_time` is a model's
claim about when to post, not a measurement of when this creator's audience is
online — nothing here measures that — so it prefills a field the creator can
change and names where the time came from. An unparseable one ("evening",
"after work") prefills NOTHING rather than a plausible default: a specific time
in a field with no explanation reads as a decision the product made.
**It never schedules into the past**, at prefill (a plan suggesting 5pm opened
at 8pm means tomorrow at 5pm) *and* at save, because the creator can type a past
time and a post scheduled for yesterday never fires. Dates format in LOCAL time
rather than `toISOString().slice(0,10)`, which is UTC and lands on the wrong day
for anyone west of Greenwich after their afternoon.
**And it states what scheduling actually does** — a calendar entry with the
caption ready, auto-posting only for a connected platform. Claiming otherwise is
how someone misses a launch.
*Still to build:* first-frame selection (choosing a frame FROM the video, as
distinct from the AI cover — it needs canvas access to a signed URL, and a
tainted-canvas failure is not verifiable from here), and aspect/safe-zone
variants (a render change, so it needs the editor running).
...
16. Creative Transfer page with honest OBSERVED / NOT OBSERVED labels
    ...
    **BUILT** — `creativeTransferRows.ts` + `CreativeTransfer.tsx`, on the plan
    screen's "Why it works" tab.
    **§1.1's machinery already existed and is careful.** `referenceEvidence.ts`
    lists the nine things we never look at (`MISSING_EVIDENCE_TYPES`) and emits
    each as `unknown` with a reason, and `creativeTransferPlan.ts` refuses to
    let an `unknown` support a transfer. What did not exist was anything that
    SHOWED it. Fourth time this session a "missing" thing was already shipped.
    **§1.1'S OWN EXAMPLE OVER-CLAIMS, by this codebase's standard.** It writes
    *"Story structure OBSERVED from transcript beats"* — but `narrative_beat`,
    `format_label`, `hook_window_sec`, `stated_cta` and `why_it_works_claim` are
    all filed as **interpreted**: a model's reading of a transcript, not a
    measurement of one. Only the platform and language are `observed`. So the
    rows carry all four kinds rather than §1.1's two; collapsing them back would
    upgrade "a model read it this way" to "we observed this" — committing §1.1's
    over-claim while implementing §1.1.
    **A HOLE CLOSED IN THE PLAN VALIDATOR.** `evidenceIds` was constrained on
    unused actions and `observedTraits` was not, so a `brand_default` or
    `reject` decision could report any traits at all with nothing behind them —
    and traits are exactly what this screen renders. *"Reference used physical
    comedy"*, on a rejected dimension, is §1.1's fabricated row arriving through
    the one action nobody checked, because rejecting felt like the safe
    direction. A decision that used nothing observed nothing it can show.
    **It reads the blueprint, not a plan, because NOTHING WRITES A PLAN.**
    `creative_transfer_plans` ships with a contract, a semantic validator and a
    migration and has no producer; a page requiring one would show an empty
    state to every creator forever. The normalizer is pure over
    (structure, transcript) and `reference_read` carries the structure half on
    every generation, so the rows are real today.
    *Still to build:* the planner that writes a plan, and the server-side rows
    (`measured` duration, language and word timings live server-side, so those
    rows are absent here — an absence, not a zero).
...
**PRODUCTION-MODE MATCH IS NOW BUILT** — 0106 gave it somewhere to store an
answer, and `galleryRank.ts` reads it. The design is entirely in what it refuses
to conclude: it answers ONLY where both halves are explicitly known — the card
assessed (0106's columns), and the creator ASKED (0103's flags). A mismatch
therefore carries two pieces of evidence, and anything less is `not_checked`,
because the alternative is telling a creator they cannot make a video on the
strength of two absences.
**UNSET IS NEVER FALSE, ON EITHER SIDE, and the numbers make the point**: all
**6,608** gallery cards are NULL today. Had the column defaulted to false, every
one of them would claim "needs no objects to film", and a creator who cannot
film objects would be told the whole gallery suits them. On the creator side the
same rule means somebody who skipped the onboarding question is not treated as
unable — which would hide most of the gallery from them.
The gallery is therefore the FIRST CONSUMER of the capability flags, and
`CAPABILITY_CONSUMERS_BUILT` now names both filming flags. The pinning test did
its job: it failed the moment a consumer landed, exactly as its comment
promised. `needs_approval` remains unread.
*Still to build:* the five other signals, each behind the field named in the
code — and a WRITER for 0106's columns, since the discovery scraper is what will
assess a card and nothing does yet. Until then the signal is honest and almost
always unanswered.
...
| 2026-08-05 | **Phase 13 item 16 (Creative Transfer) BUILT — §1.1's trust screen, and the two things checking first turned up.** The machinery already existed and is careful: `referenceEvidence.ts` lists the nine things we never look at and emits each as `unknown` with a reason, and the plan validator refuses to let an `unknown` support a transfer. Nothing showed it. **§1.1's own example table over-claims by this codebase's standard** — it writes "Story structure OBSERVED from transcript beats", but `narrative_beat`, `format_label`, `hook_window_sec`, `stated_cta` and `why_it_works_claim` are all filed as INTERPRETED, a model's reading of a transcript rather than a measurement of one, and only the platform and language are `observed`. The rows therefore carry all four kinds; collapsing them back to §1.1's two would upgrade "a model read it this way" to "we observed this", committing §1.1's over-claim while implementing §1.1. **And a hole was closed in the plan validator**: `evidenceIds` was constrained on unused actions and `observedTraits` was not, so a `brand_default` or `reject` decision could report arbitrary traits with nothing behind them — and traits are exactly what a trust screen renders, so "reference used physical comedy" on a rejected dimension is §1.1's fabricated row arriving through the one action nobody checked, because rejecting felt like the safe direction. The existing control test was already clearing every other trace of reference use and not that one. The page reads the blueprint rather than a persisted plan because `creative_transfer_plans` ships with a contract, a validator and a migration and NO PRODUCER; requiring one would show an empty state to every creator forever, and the normalizer is pure over (structure, transcript) with `reference_read` carrying the structure half on every generation. The nine never-looked-at rows are shown whether or not anything was extracted — a shorter list on a thinner reference would let absence look like completeness, which is the failure the screen exists to prevent. |
| 2026-08-05 | **Phase 12 item 12 (container resolution) BUILT, and the leak it closes was being read into the camera.** Two independent placeholder repairers existed — `Result.tsx` and `recordingScriptAdapter.ts` — with near-identical regexes, and both only matched a bracket token that was the ENTIRE line. "Today I want to talk about [product name]" was a placeholder to neither: it survived both, reached the teleprompter and was read aloud. The two had also drifted, one trimming and one not, so the same line was a placeholder on the plan screen and real dialogue in the recorder. They are now one authority. **A declared clip is not an unresolved slot:** `[SHOW: …]` is bracket-shaped and is the deliberate marker item 11 is built on, so a resolver that stripped every bracket would delete it while reporting success — and the old whole-line rule would have replaced a lone `[SHOW: …]` with the hook text. It is classified separately, matched case-insensitively because a marker that only counts in capitals silently becomes a gap, and left exactly as written. **Only the hook is filled**, being the only slot whose correct value this system holds; everything else is shown and never guessed, because §2.3's absolute rule is that an unverified fact never reaches a script and the alternative to a visible "[product name]" is an invented product name. There is deliberately no auto-fill button. **It reports rather than blocking the record button** — a hard gate would strand a creator whose script has one stray bracket with nothing to do about it, and stranding someone at the camera is what Phase 11 exists to prevent; the gap is named on the plan screen beside the script editor that fixes it in one edit. *Still to build:* the FALLBACKS. §2.3's three routes (brand-led, reference-led, research-led) are what would FILL a container rather than report it, and none exists — so this closes the "nothing unresolved reaches filming" half and declares the other half rather than implying it. |
| 2026-08-05 | **Phase 12a started: the cover moved behind a button, and generate-once became a real guarantee.** `generate-thumbnail` had NO idempotence check — every invoke called the paid image model and wrote a new `Date.now()`-suffixed object, and the only thing preventing a second call was a client-side `!gen.ai_thumb_path` conditional deciding whether to render the button. A second tab, a stale cache, a direct invoke, or the moment before the generation loads each defeated it, and every miss both cost a paid call and silently replaced the creator's cover. The function now short-circuits on a stored path (re-sign, no model call, no write, and deliberately no `force`), **and viewing does not invoke it at all**: an existing cover is signed client-side via `signEditUrls`, which covers being stored at `${user.id}/ai-thumb/…` was already designed to allow. That second part closes a deploy window — the edge function auto-deploys on merge while the web app deploys from the same push, so a new client can meet an old function, and in that window "open the cover" would have regenerated it. Viewing is a read, so it goes down a read path. The cover itself was a permanent 9:16 card on the plan screen; it is a thing wanted exactly twice and it occupied a screen the creator reads while filming, so it is now a button beside Download that opens a dialog with the image and a download link, and there is no Regenerate anywhere. Caption and hashtags now copy SEPARATELY, because the platforms take two fields and Instagram creators routinely put tags in the first comment — a combined copy made them hand-delete tags every time — and each button reports its own copy rather than sharing a flag that would claim the hashtags were copied when the caption was. The row states length against the platform's own published limit: a checkable fact, never advice about what length performs, and a platform not in the map shows nothing rather than a guessed limit. |
| 2026-08-05 | **Phase 12a: scheduling wired from the plan screen.** Every piece already existed — `schedulePost`, the calendar, the `publish_due` cron — and the plan screen was simply not connected to any of it: `best time: 5:00 PM` rendered as dead text beside a "Mark as posted" button that only works after the fact. Each publish row now has a Schedule button. **The prefill is a suggestion and the screen says so**: `best_time` is a model's claim about when to post, not a measurement of when this creator's audience is online — nothing here measures that — so it prefills a field the creator can change and names where the time came from, and an unparseable one ("evening", "after work") prefills NOTHING rather than a plausible default, because a specific time sitting in a field with no explanation reads as a decision the product made. **It never schedules into the past**, at prefill and again at save — the creator can type a past time, and a post scheduled for yesterday never fires. Dates format in LOCAL time rather than `toISOString().slice(0,10)`, which is UTC and lands on the wrong day for anyone west of Greenwich after their afternoon. The dialog also states what scheduling actually does — a calendar entry with the caption ready, auto-posting only for a connected platform — because claiming otherwise is how someone misses a launch. |
| 2026-08-05 | **0106 — the two things that had nowhere to live.** Phase 12 item 13 (screen recording as a clip type) and §7a's blocked production-mode signal both stopped at the same wall: `media_assets.kind` was a closed list with no clip in it, and no column carried what a REFERENCE requires to shoot. Neither was a design question; both were one field. **Adding a kind is safe here for a checkable reason rather than a hopeful one:** every existing consumer names `'source'` explicitly, so nothing widens to pick a clip up, and 0091's `source_validate_not_source` already refuses to run the source validator over a non-source — which is the behaviour wanted, since that validator enforces properties of a recorded TAKE. The idempotency backbone extends to clips deliberately: a screen capture is exactly where a creator retries (picker cancelled, wrong window, tab refreshed), so one attempt is one asset there too — without it three cancelled attempts leave three half-uploaded rows and the creator picks between them. `clip_label` is how item 11's `[SHOW: …]` gets name-matched, and NULL means an unattached clip (a legitimate state) rather than a clip that fills every slot. **On the gallery side, NULL is the point again**: three states exactly as 0103's capability flags, because the gallery is full of scraped rows nobody has assessed and defaulting them to false would tell a creator who cannot film objects that every unassessed reference is fine for them — the missing-value-read-as-real failure in the place §7a says it costs most. An assessment must name its source (`human` ≠ `model`, the distinction `referenceEvidence.ts` spends its taxonomy on), enforced as an EQUIVALENCE so a source with no assessment is refused too. **Gate-J proves both, and it caught a real bug in itself first**: the pre-state check runs an insert that is expected to FAIL, and under `set -o pipefail` that aborted the gate before it started — it failed loudly rather than passing silently. The silent-failure case each addition has is what the gate is for: the old kind CHECK was inline, so a migration guessing its name would drop nothing, add a second check beside the surviving restrictive one, and report success while `clip` stayed refused. A count assertion proves exactly one survives. |
| 2026-08-05 | **§7a's production-mode match BUILT, and 0102-0106 APPLIED to production.** The migrations went on directly (0 edit_projects existed, so 0102's new status-transition trigger had nothing to strand) and were verified against the database rather than trusted: 4/4 tables, 2/2 capability columns, 3/3 functions, 3/3 claim constraints, 7 policies, `anon` refused on `dna_claims`, and the n=1 rule reading correctly in production as `CHECK (claim_type <> 'correlation' OR (sample_size IS NOT NULL AND sample_size >= 2))`. 0106's silent-failure case was checked before and after: production's inline constraint really was named `media_assets_kind_check`, and exactly ONE kind CHECK survives the swap. **The signal itself answers only where BOTH halves are explicitly known** — the card assessed and the creator asked — so a mismatch carries two pieces of evidence and everything else is `not_checked`, because the alternative is telling a creator they cannot make a video on the strength of two absences. **The production numbers vindicate the NULL decision concretely: all 6,608 gallery cards are unassessed.** A `false` default would have had every one of them claiming "needs no objects to film", telling a creator who cannot film objects that the entire gallery suits them — silently. The gallery is now the FIRST consumer of the capability flags, and the test pinning `CAPABILITY_CONSUMERS_BUILT` to an empty list failed the moment that landed, exactly as its own comment promised it would. *Still to build:* a WRITER for 0106's columns — the discovery scraper is what will assess a card, and nothing does yet, so the signal is honest and almost always unanswered. |
| 2026-08-05 | **The brief was being asked and thrown away — and three things that existed without connecting.** A production audit, run against the deployed database rather than against this document. **The brief:** `Onboarding.tsx` collected `workKind` and `forbiddenClaims`, wrote them into a `localStorage` draft, and persisted them nowhere — no column existed, and grepping the tree for `work_kind` or `forbidden_claims` returned nothing. A doctor typed what they may never claim into a box we put in front of them and it lived in one browser until that browser was cleared. Asking and discarding is worse than not asking: an unasked question leaves a creator knowing the system does not know, and a dropped one leaves them believing it does, which is why they stop checking the output for the claim they told us never to make. `0109` stores it beside the brand voice `generate-blueprint` already loads, so it does not become a second authority for facts that function resolves by inference today; the creator's own answer now wins over the scan's reading, and the claims answer reaches the prompt as a constraint — emitted ONLY when there is one, because a section reading "restrictions: none" would be this system answering a question nobody asked. Gate-L proves the CHECK against real Postgres, including the mutation control that an empty string gets in without it. **Preflight:** Phase 11 item 6, this document's named #1 gap, had been fully built and tested since it landed and was imported by NOTHING. The measurer now reports what a browser can honestly observe (orientation from the video track; whether a live audio track exists, and its peak) and refuses the rest — framing needs a face box, lighting needs a subject region, reverb needs a decay measurement against a known excitation, and a centre-of-frame proxy would be wrong exactly when the creator is off-centre, which is the case the check exists to catch. That forced one contract correction: `PreflightSignals.audio` required peak AND reverb together, so a measurer holding one had to fabricate the other — and a fabricated reverb of 0 sits under the ceiling and renders as a green "room sounds fine" tick for a room nobody heard. The two halves are now independently optional. The panel draws "not checked" rather than omitting it, and reports without blocking the record button, on `UnfilledContainers`' precedent. **Placement**, listed as unbuilt one entry above, is built: `sceneOutputWindow` maps a scene through the surviving cut and `placeClips` trims a long clip to its window (never retimes — that would make the plan's 1x guarantee false), lets a short one end and the take come back, and drops the later of two clips fighting over one stretch with a warning naming it. `0108` stores the scene number on the clip because the pinned script snapshot carries no `clip_label` and adding one moves a SHA that binds every take in flight, while the live `scene_timeline` may have changed since the clip was recorded. **What the audit found that is NOT yet fixed, recorded rather than left to be rediscovered:** `enqueue-autoedit` is deployed to production, version 37, from a `/tmp` entrypoint, with no directory in this repository and no commit that ever touched one — unreviewable, unversioned, and only harmless because `reject_new_autoedit_job` refuses its jobs at the database. `deploy-edge.yml` now reads the DEPLOYED set and names anything nobody committed, warning for that one and failing for any new one, because deleting a live endpoint is a decision for a person. `outcomeLog`, `referenceEvidence` and `creativeTransferRows` remain built with no consumer. And no editor-v2 project has ever run in production: 0 `media_assets`, 0 `edit_projects`, 0 `edit_plans`, 0 `edit_outputs`, against 34 profiles and 34 generations — the 17 `autoedit:done` jobs are the legacy path, which did work. Every gate this project has is a claim about the code; none of them is a claim about a video. |
| 2026-08-05 | **Phase 12 item 13 — screen recording, end to end, and the one piece deliberately left to an eval.** The blocker PR #283 named was real and is gone: `EditPlanV1` gained `composition` (schema v7), so a plan can name screen-capture clips and where they appear. It is **not a second timeline** — the take stays the spine, `source`/`timeline`/the time map are untouched, a clip is only ever PICTURE (its audio discarded, always, because two audio sources need ducking and a loudness decision that do not exist), and a clip plays at 1x, enforced, because unequal source and output spans are a speed change and a speed change is a second time map wearing a disguise. The slot a clip fills is carried as a DIGEST, which makes `composition` the first section in the document with no exemption from the metacharacter scan. **The graph cuts away without a comma**, built exactly the way the zooms are and for the same reason: `enable=between(t,a,b)` needs the comma `VALUE_RE` refuses, so the timeline is split into base/clip windows, each conformed identically (`concat` rejects inputs disagreeing on fps, timebase, SAR or pixel format) and joined in order — after the zooms (a punch chosen for a face means nothing on an interface) and before the captions (the creator is still speaking; burning them first lets the cutaway paint over the line being said underneath). A plan composing nothing gains ZERO nodes, so no existing hard-cut plan's graph hash moved. **Three defects the connectivity audit named, all in this track, all fixed:** `[SHOW: …]` was reaching the teleprompter as dialogue — the creator was being prompted to read "show: the settings page" into the camera, because `isWhollyPlaceholder` deliberately refuses to call a declared clip a placeholder and nothing downstream reclassified it; the capture surface derived its slots from `blueprint.script[]` while the editor and teleprompter use `scene_timeline`, so a slot edited OUT was still offered and one added never appeared; and `clip-asset` was in neither list in `deploy-edge.yml`, which fails on unclassified directories by design — the function could never have deployed. **A clip is verified against the checksum THE PLAN pinned, not the asset row's.** They agree until a creator re-records a clip under the same label, at which point the row moves and the plan does not — and if the row won, a render would composite footage the plan never described with the plan hash, the graph hash and the manifest all still correct. `render_graph_invalid` is now classified in the failure catalogue: it existed only as an `EditPlanError`, so the coverage test pinning "no creator hits a failure nobody classified" had never seen it, and throwing it as a real `PermanentJobError` made it fail immediately — exactly what it is for. *Deliberately not built:* **WHERE an overlay goes.** A declared clip is shown while the creator keeps talking, so its window falls over neighbouring dialogue — an editorial decision, and the natural shape is a `DirectorClipRequest` on `DirectorZoomRequest`'s precedent (the model picks WHICH clip and WHICH anchor word; the compiler computes the window from the time map; the duration is the clip's own measured length, never a model number). It is not built because it would change the envelope AND the response schema, therefore the Director bundle identity — and `DIRECTOR_SEES_ALIGNMENT`'s comment already settled what to do here: *"this project has a director-eval harness and a quality gate precisely because that is not a judgement to make by reasoning."* So the compiler emitted an empty composition on purpose, the render stage refused a composing plan by naming the missing half, and the capture surface told the creator their clips were saved but not yet cut in. Three layers saying it rather than one hiding it. |
| 2026-08-06 | **Superseding the paragraph above: clips ARE placed, and the three layers had to be corrected together.** `placeClips` landed in `editorCompile.ts` and `editorCompileStage.ts` loads the generation's ready clips into it, so a plan now carries a real `composition` and the renderer composites it. What the entry above describes as deliberate — an empty composition, a render stage that refuses, a capture surface saying "not yet" — was accurate when written and became false the moment the compiler could place. The UI paragraph in `DeclaredClips.tsx` kept telling creators their captures would not appear for as long as it took someone to notice, which is the specific harm of a stale promise: a creator told their clip would NOT be used has no reason to check that it was used correctly. Placement is DETERMINISTIC BY SCENE WINDOW, not chosen by the Director — a clip fills the output window of the scene it was declared against (`clip_scene_number`, 0108), is trimmed to that window if longer, ends early if shorter, and is dropped with a `clip_unplaced` warning if the scene did not survive the edit, if the window is too short, if it duplicates another clip, or if a second clip contests the same span. The `DirectorClipRequest` the entry above describes as the natural shape is still unbuilt, and the distinction matters: WHICH line a clip plays over is the creator's declaration, not a model's judgement. That is a defensible rule, but it is a rule this row is now stating rather than a behaviour the compiler happens to have. **Still unproven: no real-media run has confirmed a clip appears at the intended scene in a rendered file.** |

---

## §N — Niche Brain and end-to-end data wiring (2026-09-25)

**Why this section exists.** The owner asked for proof that every mode pulls the right information in and writes the right information back. It records what each source feeds, where it is read, and how it was verified. "Verified in code" means the read or write exists on the live path; "verified live" means production rows show it happening.

### What each script is built from (generate-blueprint)

| Source | Idea mode | Reference mode | Product mode | Read at | Status |
|---|---|---|---|---|---|
| Creator DNA (`brand_voices.profile`) | ✅ | ✅ | ✅ | `from('brand_voices')` | verified live (every script) |
| Her paragraph (`reference_note`) | ✅ primary input | optional note | usually just the product name | request body | verified live |
| The reference video (transcript + structure) | — | ✅ primary input | — | `from('transcripts')` | verified live |
| **Product brief (0225 stories, summary, offer, rotating angle)** | when a product is picked | when a product is picked | ✅ **primary input (new)** | `productModeBlock` | verified in code; live check on the next product run |
| Product facts (placed and trust-gated) | ✅ | ✅ | ✅ | `from('product_entities')` / `placeFacts` | verified live |
| Availability (0234, daily shop stock) | ✅ | ✅ | ✅ | `availabilityBlock` + sold-out CTA swap | verified in code; worker filling |
| Her knowledge (`creator_knowledge`) | ✅ | ✅ | ✅ **scoped to the selected product** | `readKnowledge` + `knowledge_scoped_to_product` | verified in code |
| Niche patterns (brain notes, private and shared) | ✅ | ✅ | ✅ | `brain_brief_scoped` | verified live (notes read and used) |
| Rising in her lane | ✅ | ✅ | ✅ | `brain_trends` | verified live (RPC returns rows) |
| World moments (daily grounded Google) | ✅ | ✅ | ✅ | `brain_moments` | verified live (30 moments, 5 buckets) |
| Her track record (best/weakest posts, scripts already written, posted) | ✅ | ✅ | ✅ | `creator_track_record` | verified live |
| Her edits to earlier scripts | ✅ | ✅ | ✅ | `creator_recent_edits` | verified in code (1 edit in prod) |
| Her ratings of earlier scripts | ✅ | ✅ | ✅ | `creator_script_feedback` | verified in code (awaiting first rating) |
| Caption-shape / cohort evidence (gallery) | ✅ | ✅ | ✅ | `from('gallery_items')` | verified live |
| Niche vocabulary | ✅ | ✅ | ✅ | `niche_vocabulary` | verified live |

Every brain read runs in parallel under one 2.5 s cap and fails open. Any error leaves the prompt exactly as it was before the brain existed.

### What writes back (the three learning loops)

| Loop | Written by | Read by |
|---|---|---|
| Niche | worker `nicheBrain/sweep.ts` → `corpus_reads`, `brain_notes`, `brain_links` | `brain_brief_scoped`, `brain_trends`, What Twin knows, Ideas |
| Hers | worker `runOwnPostSweep` → `own_post_reads`, private `brain_notes` | same, owner-scoped |
| Script | `brain_note_uses` (each run) + `brain_learn()` every 30 min (filmed = recorded take / approved output / edit project / answer; posted; views; stars) | `brain_brief_scoped` ranking |
| Her feedback | `script_ratings` (RateThisScript), `script_edits` | `HER TRACK RECORD` in the prompt |

### Surfaces the creator sees

- **Create:** Ideas for you (`creator_ideas`, written daily per voice).
- **Result:** RateThisScript and DidYouFilmIt.
- **What Twin knows** (`/brain`): world moments, patterns from her own posts, and niche patterns.
- **Products:** sold-out and partly sold-out badges.

### Root cause recorded (owner, 2026-09-25)

**Idea mode receives new material (her paragraph). Reference mode receives new structure (the video). Product mode received neither.** Its note was just the product's name. That is the measured cause of the audit's fabrication ("22 pieces", "alongside a 9-5", "thirty minutes") and cross-entity leakage (the bowl-vs-mug pricing comparison in mug-only, tool and idea scripts). The fix is the Product brief plus product-scoped knowledge above.

**Still to verify live:** after the next Petal Bowl and Ceramic Mug runs, recount invented numbers, leakage, teleprompter/shot-list desync, and the share of scripts that end on `set_down` (baseline: 15 of 87, 17%).
