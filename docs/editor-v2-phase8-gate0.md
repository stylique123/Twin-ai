...
- **Stateful prerequisites** — the VPS capacity inventory and the migration ledger
  ...
  staging run, candidate acceptance, merge, and deployment. **Status 2026-07-28: BOTH CLOSED.** The VPS row
  closed with #220, the ledger row with #212. Neither was waived — each moved because
  the work was done and measured, and §10 carries the evidence. Offline work is no
  longer gated by a stateful prerequisite.
...
| **Migration ledger reconciliation** (STATEFUL) | 🟢 CLOSED — #212 merged (`dd8acfa7`). Expected state built in an isolated disposable Postgres from byte-exact committed migrations; staging read read-only; missing, modified AND additive drift all fail closed. 25 hostile cases through the real guard. The previous algorithm remains DISABLED and is not reused. Parity is deliberately narrow — 0090-0093 only. |
