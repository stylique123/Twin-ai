...
| `UNVERIFIED_GEMINI_API_KEY_EXPOSURE_CLAIM` | A system audit dated 2026-08-26 asserted that a `GEMINI_API_KEY` had been pasted in plaintext into a session transcript and needed rotation. The claim could be neither confirmed nor refuted from here; what was measured on 2026-08-26 was zero `AIza`-shaped strings in the worktree, zero across every ref in git history, and zero in the one transcript present on that machine. One transcript is not every session, so absence there was never absence. | The next time the owner was in the Google AI Studio console for any reason. | **RESOLVED 2026-09-15 by rotation, outcome (a).** A new key was issued and the old one deleted in Google AI Studio, so the pasted credential no longer authenticates. Places updated: **Supabase edge function secrets** (`generate-blueprint:5809`, `generate-thumbnail:28`, `_shared/dna.ts:602`), **GitHub Actions secret `GEMINI_API_KEY`** (`staging-integration.yml:59`, `pr-checks.yml:1071`), and the **worker VPS** `/opt/twinai-worker.env` (`worker/src/env.ts:24`). ⚠️ WHAT WAS SEEN VERSUS REPORTED, because this row is worth nothing if it blurs them. SEEN from this environment: the VPS key's sha256 prefix changed `d25b0c50` -> `11382f20` across the redeploy, the new value returns 200 from `/v1beta/models`, and the worker came up at 23:14:21 with `GEMINI_API_KEY: true`. REPORTED by the owner and not verifiable from here: that the old key was deleted, and that the Supabase and GitHub copies hold the new value. ⚠️ THE CREATION DATE AND LAST-USE THIS ROW ASKED FOR WERE NOT RECORDED, so the original question -- whether the 2026-08-26 audit was right -- stays unanswered. Rotation closes the exposure either way, which is why this is `RESOLVED` rather than left open. ⚠️ AND THIS ROW'S OWN 'places' LIST WAS INCOMPLETE UNTIL TODAY: the standing note claimed `worker/src/env.ts:24` was the ONLY reader of `GEMINI_API_KEY`. Measured 2026-09-15, there are THREE homes, all named above. A rotation following the old list would have left the edge functions and CI on a dead key. | `RESOLVED` |
