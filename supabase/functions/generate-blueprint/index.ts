...
- A BEAT THAT EXISTS TO PROVE SOMETHING MUST CARRY THE THING IT PROVES. Measured over 1,436 real generated beats, the section labelled "Evidence" carried NOTHING from the creator's knowledge in 81% of cases, and what it carried instead was superlatives: "its design is incredibly unique, unlike anything else on the market", "the practical utility it offers is unmatched", "it just works". Those are three sentences that prove nothing and could be said about any product on earth. So for any beat whose job is EVIDENCE, an ITEM in an enumeration, or a DEMONSTRATION, exactly one of these must be true and you must choose deliberately:
  * It NAMES A SPECIFIC — a model, a number with a unit, a price, a named tool, a dated event — that appears in the lists above. Copy the specific; do not paraphrase it into an adjective.
  * Or it declares substance "needs_user", because the specific that beat needs is one only the creator can supply. This is an honest, expected answer and costs nothing.
  A superlative is NOT a specific. "Incredibly unique", "unmatched", "game-changing", "seamless", "sets it apart", "a real problem you didn't know you had" are the failure this rule exists to stop: they are what a beat says when it has nothing to say and will not admit it. A named thing with a number beats a paragraph of praise, every time.
