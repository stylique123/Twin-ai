...
function spelledOutValues(text: string): string[] {
...
function claimedValues(text: string): Set<string> {
  ...
  for (const m of String(text ?? '').matchAll(CLAIM_VALUE)) add(m[0])
  // ⚠️ THE DIGIT TEST ABOVE IS WHY THIS SECOND PASS EXISTS RATHER THAN A WIDER
  // `CLAIM_VALUE`. A spelled-out figure has no digit to survive it, so it is
  // converted to digits FIRST and then put through the same canonicaliser — one
  // canonical form for both notations, by construction.
...
// ⚠️ TWO INDEPENDENT ADDITIONS LANDED AT THE SAME INSERTION POINT, and git
// could not know they do not overlap. The disclosure rule below and the
// duration contract after it share nothing but a line number: one decides
// whether a paid tie was declared, the other how long the video runs. BOTH
// ARE KEPT, in that order. Resolving this by choosing a side would have
// silently deleted a shipped feature.
...
// ⚠️ THIS CLOSING BRACE IS THE ONE THE CONFLICT ATE. The `}` that followed the
// `>>>>>>>` marker closed whichever side won, so keeping BOTH blocks left
// `scriptDisclosesInline` unterminated — and the only symptom was esbuild
// saying "Unexpected end of file" 6,400 lines later. `check_edge_functions_parse`
// caught it; tsc alone would not have been enough to trust here.
}
...
- hook_options: give 5, ordered best first. The FIRST one is your recommended pick. Each hook is one spoken line, scroll-stopping, and must visibly stack at least two of the four triggers above.
- HOOK LENGTH IS A COUNT, NOT A FEELING. Every one of the five is 12 words or fewer. Count the words of each hook and check it before moving on; at 13 or more, cut words until it fits — never round up to "close enough". A hook over 14 words is automatically DEMOTED below the ones that comply, so a long hook is not a bolder hook, it is the one the creator sees last.
