...
export interface FactLine {
  ...
  stated: boolean
  /**
   * The provenance marker alone, for a caller that renders its own line.
   *
   * ⚠️ EXPORTED SO THE CALLER CANNOT RE-SPELL IT. generate-blueprint builds the
   * CREATOR DNA block as one template literal with per-field trailing
   * fragments, so it appends this rather than using `line`. If it wrote its own
   * wording, the two would drift and the guard below could not check either.
   */
  suffix: string
...
export function businessFactLines(snapshot: BrandTruthSnapshotV1 | null): FactLine[] {
  ...
  for (const [field, label] of DNA_BUSINESS_FACTS) {
    ...
    if (!got) continue
    const suffix = got.stated ? STATED_SUFFIX : GUESSED_SUFFIX
    out.push({
      field,
      label,
      value: got.value,
