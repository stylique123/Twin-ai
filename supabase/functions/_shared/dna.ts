...
// Mirror of packages/shared/src/handleShape.ts — pinned by handleShape.test.ts.
...
//
// Inlined from `packages/shared/src/transcriptSelection.ts` (Deno deploy cannot
// import @twinai/shared), where the rationale and 9 tests live.
// `transcriptSelectionParity.test.ts` fails if the two drift.
//
// ⚠️ THIS TOOK THE TOP FIVE BY REACH, WHICH SAMPLES ONE THING: WHAT WENT VIRAL.
// Viral videos are systematically the least representative source of belief —
// they skew to spectacle and to older uploads that had time to accumulate
// reach. Transcripts are the ONLY source that can produce `stated` positions
// (captions are clamped to `demonstrated`, which resolves to coverage), so
// WHICH FEW get transcribed decides whether Twin can ever say anything the
// creator actually believes.
//
// ⚖️ The permalink preference below is UNCHANGED and load-bearing: Instagram's
...
export function extractVideoUrls(items: Record<string, unknown>[], max = TRANSCRIPT_BUDGET): string[] {
  const seen = new Set<string>()
  const candidates: TranscriptCandidate[] = (items ?? [])
    .map((it) => ({
      url: pick(it, ['webVideoUrl', 'url', 'postUrl', 'video.url', 'videoUrl']),
      plays: pickNum(it, ['playCount', 'videoViewCount', 'viewCount', 'views', 'diggCount', 'likesCount', 'stats.playCount']),
      text: String(pick(it, ['text', 'caption', 'title', 'description']) ?? ''),
      postedAt: (pick(it, ['createTimeISO', 'timestamp', 'publishedAt', 'date', 'createTime']) as string) || null,
    }))
    .filter((r) => /^https:\/\//i.test(r.url))
    .filter((r) => (seen.has(r.url) ? false : (seen.add(r.url), true)))
  return selectVideosToTranscribe(candidates, max)
}
