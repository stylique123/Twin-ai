// GENERATED — DO NOT EDIT. Byte-for-byte copy of
// packages/shared/src/brandTruth.ts below this banner.
//
// WHY A COPY AND NOT AN IMPORT. The Supabase edge bundler resolves imports from
// the function directory; a relative path out to `packages/shared` does not
// deploy. `_shared/` is how this tree already solves that (`dna.ts`,
// `outputLinks.ts`), and the worker solves the same problem the same way
// (`worker/src/jobs/brandSnapshot.ts`, a mirror with a parity test).
//
// WHY A COPY AND NOT A RETYPING. Those existing mirrors were retyped by hand,
// which is where drift gets in. This module imports NOTHING and touches no
// runtime API — it produces a canonical string and leaves hashing to its
// caller — so the copy can be exact, and `scripts/ci/check_brand_truth_parity.mjs`
// fails the build the moment it stops being.
//
// To update: change the shared file, then re-run the guard, which prints the
// copy command.
