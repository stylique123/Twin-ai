...
async function main() {
  ...
  console.log('\n== S. real transcription of a real recording ==')
  {
    ...
    check('S2 exactly ONE speech component (asset × speech × speech-3)',
      rows.length === 1 && rows[0].analyzer_bundle_version === 'speech-3', `rows=${rows.length}`)
    const r = rows[0]?.result ?? {}
    check('S3 component identity: schema 1, checksum bound to the validated bytes',
      r.schemaVersion === 1 && r.speechVersion === 'speech-3' && r.sourceChecksum === S.asset.content_sha256)
    ...
    check('S16 analyzing re-verified the durable component',
      ver?.details?.word_count === words.length && ver?.details?.speech_version === 'speech-3', JSON.stringify(ver?.details))
  ...
  {
    const pid = await startProject(cA, S.gen, S.assetId)
    const proj = await runToSettled('p5-v2', pid, { EDITOR_SPEECH_VERSION: 'speech-4' })
    check('V1 bumped-version project completed', proj.status === 'completed', proj.status)
    const rows = await speechRows(S.assetId)
    check('V2 a NEW immutable component for speech-4 (both rows kept)',
      rows.length === 2 && new Set(rows.map((x) => x.analyzer_bundle_version)).size === 2, `rows=${rows.length}`)
    check('V3 recording speech-4 did not stamp the inspection epoch',
      proj.analysis_version !== 'speech-4', String(proj.analysis_version))
